...
function PosterCard({ item, onPress }) {
  ...
  const ratingLabel = ratingValue != null && ratingValue !== ""
    ...
    : null;
  return (
    <YStack width={200} cursor="pointer" onPress={() => onPress(item)} pressStyle={{ opacity: 0.8 }} hoverStyle={{ scale: 1.03 }} animation="quick" {...({ className: "lumen-poster", "data-item-id": String(item.stream_id) })}>
...
/* ─── Shelf — lazy-loads when visible via IntersectionObserver ─── */
      ...
      {items === null ? (
        ...
        <div style={{ position: "relative" }} className="lumen-shelf-rail">
          ...
          <div ref={railRef} onScroll={handleScroll} style={{ display: "flex", overflowX: "auto", gap: 8, paddingLeft: 48, paddingRight: 48, scrollbarWidth: "none", msOverflowStyle: "none", cursor: "grab" }}>
            {items.map((item) => <PosterCard key={String(item.stream_id)} item={item} onPress={onPlay} />)}
            {loadingMore && (
      ...
      {!displayed ? (
        ...
        <ScrollView flex={1} minHeight={0} contentContainerStyle={{ paddingHorizontal: 48, paddingVertical: 32 }} onScroll={handleScroll} scrollEventThrottle={200}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, 200px)", gap: 12, justifyContent: "center" }}>
            {displayed.map((item) => <PosterCard key={String(item.stream_id)} item={item} onPress={onPlay} />)}
          </div>
          {(hasMore || loadingMore) && (
            <YStack alignItems="center" paddingVertical={24}><Spinner size="small" color="#e94560" /></YStack>
          )}
        </ScrollView>
      )}
    </YStack>
...
export default function MoviesScreen({ navigation }) {
  ...
  const handleShelfVisible = useCallback(async (catId) => {
    ...
    try {
      ...
      } else { const streams = await iptvApi.getVODStreams(catId); all = streams || []; }
      ...
      <ScrollView flex={1} contentContainerStyle={{ paddingBottom: 80 }}>
        ...
        <YStack paddingTop={0}>
          {shelves.length > 0 ? (
            ...
            <YStack padding={60} alignItems="center"><Text color="#666" fontSize={15}>No movies found</Text></YStack>
          )}
        </YStack>
      </ScrollView>
      {currentCategory && (
