...
function MyListCard({ item, onPress, onRemove, focused }) {
  ...
  return (
    <YStack width={130} cursor="pointer" onPress={onPress} pressStyle={{ opacity: 0.8 }} hoverStyle={{ scale: 1.03 }} animation="quick">
      <YStack width={130} aspectRatio={2 / 3} borderRadius={8} backgroundColor={colors.surface} overflow="hidden" borderWidth={2} borderColor={focused ? colors.accent2 : "transparent"}>
        {poster
        ...
        <YStack position="absolute" top={8} left={8} zIndex={5} backgroundColor="rgba(0,0,0,0.6)" borderRadius={12} width={22} height={22} justifyContent="center" alignItems="center" cursor="pointer" onPress={(e) => { e?.stopPropagation?.(); onRemove(); }} pressStyle={{ opacity: 0.7 }} hitSlop={11}>
          <Text color={colors.text} fontSize={9} fontWeight="700">✕</Text>
        </YStack>
      </YStack>
      <Text color={colors.text} fontSize={12} fontWeight="600" marginTop={8} lineHeight={16} numberOfLines={2}>{item.name}</Text>
      {epLabel && <Text color={colors.muted} fontSize={9} marginTop={4} letterSpacing={0.3}>{epLabel}</Text>}
    </YStack>
  );
}
...
function CWCard({ item, onPress, onRemove, focused }) {
  ...
  return (
    <YStack width={260} cursor="pointer" onPress={onPress} pressStyle={{ opacity: 0.85 }} hoverStyle={{ scale: 1.02 }} animation="quick">
      <YStack width={260} height={148} borderRadius={8} backgroundColor={colors.surface} overflow="hidden" borderWidth={2} borderColor={focused ? colors.accent2 : "transparent"}>
        {bg
        ...
        {seasonBadge && (
          <YStack position="absolute" top={10} left={12} zIndex={4}>
            <Text color={colors.text} fontSize={13} fontWeight="700">{seasonBadge}</Text>
          </YStack>
        )}
        <YStack position="absolute" top={8} left={8} zIndex={5} backgroundColor="rgba(0,0,0,0.6)" borderRadius={12} width={22} height={22} justifyContent="center" alignItems="center" cursor="pointer" onPress={(e) => { e?.stopPropagation?.(); onRemove(); }} pressStyle={{ opacity: 0.7 }} hitSlop={11}>
          <Text color={colors.text} fontSize={9} fontWeight="700">✕</Text>
      ...
      <YStack paddingTop={8} paddingHorizontal={2}>
        <Text color={colors.text} fontSize={12} fontWeight="600" marginBottom={2} numberOfLines={1}>{showTitle}</Text>
        {(epLabel || epTitle) && (
          <Text color={colors.muted} fontSize={11} marginBottom={2} numberOfLines={1}>
            {[epLabel, epTitle].filter(Boolean).join(" · ")}
          </Text>
        )}
        {timeLeft && <Text color={colors.muted} fontSize={11}>{timeLeft}</Text>}
      </YStack>
    </YStack>
  );
}
...
export default function HistoryScreen({ navigation }) {
  ...
  if (myList.length === 0 && watchedHistory.length === 0) {
    return (
      <YStack flex={1} justifyContent="center" alignItems="center" backgroundColor={colors.bg} padding={24} paddingTop={insets.top + 24} paddingBottom={insets.bottom + 24}>
        <Text fontSize={48} marginBottom={12}>🎬</Text>
        <Text color={colors.text} fontSize={20} fontWeight="700" marginBottom={8}>Your list is empty</Text>
        <Text color={colors.muted} fontSize={14} textAlign="center">Open a movie and tap ♡ Favorites to save it here</Text>
      </YStack>
    );
  }
    ...
    <ScrollView flex={1} backgroundColor={colors.bg} contentContainerStyle={{ paddingTop: 24, paddingBottom: insets.bottom + 80 }} showsVerticalScrollIndicator={false}>
      {myList.length > 0 && (
        <YStack paddingBottom={40}>
          <Text color={colors.text} fontSize={22} fontWeight="700" letterSpacing={-0.3} paddingHorizontal={16} marginBottom={16}>Favorites</Text>
      ...
      {watchedHistory.length > 0 && (
        <YStack paddingBottom={40}>
          <Text color={colors.text} fontSize={22} fontWeight="700" letterSpacing={-0.3} paddingHorizontal={16} marginBottom={16}>Watch History</Text>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 16, gap: 12 }}>
