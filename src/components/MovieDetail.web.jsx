...
export default function MovieDetail({ item, onBack, onPlay }) {
  ...
  return (
    <ScrollView flex={1} backgroundColor="#0f0f23" contentContainerStyle={{ paddingBottom: 80 }}>
      {/* Hero */}
      <YStack width="100%" height={520} position="relative">
        {backdrop
        ...
        <View style={[FILL, { background: "linear-gradient(to top, #0f0f23 0%, rgba(15,15,35,0.6) 55%, rgba(15,15,35,0.15) 100%)" }]} />

        <YStack position="absolute" top={20} left={48} zIndex={10} paddingVertical={8} paddingHorizontal={14} backgroundColor="rgba(0,0,0,0.55)" borderRadius={8} cursor="pointer" onPress={onBack} pressStyle={{ opacity: 0.8 }}>
          <Text color="#e94560" fontSize={14} fontWeight="600">← Back</Text>
        </YStack>

        <YStack position="absolute" bottom={0} left={48} right={48} zIndex={5} paddingBottom={40}>
          <Text color="#fff" fontSize={40} fontWeight="900" letterSpacing={-1} marginBottom={12}>{name}</Text>

          {isLoading ? (
            <Spinner color="#e94560" marginVertical={12} />
          ) : (
            <XStack alignItems="center" gap={8} marginBottom={14} flexWrap="wrap">
          ...
          <XStack alignItems="center" gap={12} flexWrap="wrap">
            {resumeTime > 0 ? (
              <>
                <YStack backgroundColor="#fff" paddingHorizontal={28} paddingVertical={13} borderRadius={8} cursor="pointer" onPress={() => handlePlay(resumeTime)} pressStyle={{ opacity: 0.85 }} hoverStyle={{ opacity: 0.9 }} animation="quick">
                  <Text color="#000" fontSize={15} fontWeight="700">▶  Continue</Text>
                </YStack>
                <YStack backgroundColor="rgba(40,40,60,0.85)" paddingHorizontal={22} paddingVertical={13} borderRadius={8} borderWidth={1} borderColor="#3a3a5e" cursor="pointer" onPress={() => handlePlay(0)} pressStyle={{ opacity: 0.8 }} hoverStyle={{ borderColor: "#fff" }} animation="quick">
                  <Text color="#fff" fontSize={15} fontWeight="600">↺  From Start</Text>
                </YStack>
              </>
            ) : (
              <YStack backgroundColor="#fff" paddingHorizontal={28} paddingVertical={13} borderRadius={8} cursor="pointer" onPress={() => handlePlay(0)} pressStyle={{ opacity: 0.85 }} hoverStyle={{ opacity: 0.9 }} animation="quick">
                <Text color="#000" fontSize={15} fontWeight="700">▶  Play Now</Text>
              </YStack>
            )}
            {!isLoading && !!trailer && (
              <YStack backgroundColor="rgba(40,40,60,0.85)" paddingHorizontal={22} paddingVertical={13} borderRadius={8} borderWidth={1} borderColor="#3a3a5e" cursor="pointer" onPress={() => setShowTrailer((v) => !v)} pressStyle={{ opacity: 0.8 }} hoverStyle={{ borderColor: "#fff" }} animation="quick">
                <Text color="#fff" fontSize={15} fontWeight="600">{showTrailer ? "✕  Close" : "🎬  Trailer"}</Text>
              </YStack>
            )}
            <YStack
      ...
      {(data.description || data.plot || data.overview || data.cast || data.director) && (
        <YStack paddingHorizontal={48} paddingTop={24} gap={10}>
          ...
          {data.cast && <Text color="#aaa" fontSize={14} lineHeight={20}><Text color="#fff" fontWeight="700">Cast  </Text>{data.cast}</Text>}
          {data.director && <Text color="#aaa" fontSize={14} lineHeight={20}><Text color="#fff" fontWeight="700">Director  </Text>{data.director}</Text>}
        </YStack>
      )}
    </ScrollView>
  );
