...
Regenerates `../Suvo_Player_UX_Scenario.pptx`, the app walk-through LG Content
Store QA requires (LG's own `ux_scenario_document_4.4.ppt` template, filled in
for Suvo and framed as a generic BYO-playlist media player — no channel
...
node build.js                 # -> ../Suvo_Player_UX_Scenario.pptx
```
...
| `../Suvo_Player_UX_Scenario.pptx` | The deliverable to upload to Seller Lounge. |
...
```bash
soffice --headless --convert-to pdf ../Suvo_Player_UX_Scenario.pptx
pdftoppm -jpeg -r 110 Suvo_Player_UX_Scenario.pdf slide   # slide-01.jpg …
```
