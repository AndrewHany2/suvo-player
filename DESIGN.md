...
This is a *cinematic and premium* system, but premium through restraint, not spectacle. Depth is built from tonal layering — background to surface to elevated surface — not from drop shadows or glass. Motion is quiet: short, eased state changes on phone and desktop, and on the 10-foot TV, nothing moves at all (webOS Chromium on `file://` cannot afford transitions, transforms, gradients, or shadows, so the TV surface is deliberately still). The same tokens render identically across iOS, Android, web, Electron, and TV; the identity does not change per platform, only the form factor and the rendering engine do.
...
**The Literal-Hex Sync Rule.** Shared CSS must duplicate these values as literal hex, never `var()`. Old webOS Chromium can't be assumed to support custom properties, so the token file (`src/ui/tokens.js`) and the CSS mirrors carry the same hex by hand. If you change a token, change every literal that echoes it.
...
- **Don't** ship `transition`, `transform`, `linear-gradient`, `box-shadow`, or `var()` in **shared TV CSS** — webOS Chromium on `file://` can't afford them. If an effect can't render on TV, it isn't part of the shared layer.
